...
impl ExtendedKalmanFilter {
    ...
    pub fn update(&mut self, measurement: &MeasurementVector) {
        ...
        #[allow(non_snake_case)]
        let K = &self.P_ * &H.transpose() * S.try_inverse().unwrap_or_else(|| S);
    ...
    pub fn filter_pose(&mut self, x: f64, y: f64, theta: f64, dt: f64) -> (f64, f64, f64, f64, f64, f64) {
...
mod tests {
    ...
    fn test_filter_pose() {
        ...
        let (x, y, theta, vx, vy, omega) = ekf.filter_pose(1.0, 2.0, 0.0, 0.016);
        
        assert!((x - 1.0).abs() < 0.2);
        assert!((y - 2.0).abs() < 0.2);
        
        // Segunda medición (movimiento) - debería filtrar y estimar velocidades
        let (x2, y2, theta2, vx2, vy2, omega2) = ekf.filter_pose(1.1, 2.1, 0.0, 0.016);
