...
impl FIRASimClient {
    ...
    pub async fn new(
        ...
        let control_addr = Self::parse_destination(address, port)?;
    ...
    pub async fn create_robots(
        ...
        for (id, team) in robot_ids {
            ...
            eprintln!(
                "[FIRASim]   Robot ID {}: posición ({:.2}, {:.2}) m, equipo {}",
                id,
                x,
                y,
        ...
        for (id, team) in robot_ids {
            eprintln!(
                "[FIRASim]   - Robot ID {} del equipo {} ({} bytes)",
                id,
                if *team == 0 { "AZUL" } else { "AMARILLO" },
                buffer.len()
            );
        ...
        eprintln!(
            "[FIRASim] Enviando SimulatorCommand a {} ({} bytes)",
        ...
        match self.socket.send_to(&buffer, &self.control_address).await {
            Ok(bytes_sent) => {
                eprintln!(
                    "[FIRASim] ✓ Comando de creación de robots enviado ({} bytes) a {}",
                    bytes_sent, self.control_address
                );
                eprintln!(
                    "[FIRASim] Primeros 32 bytes del mensaje (hex): {:02x?}",
            ...
            Err(e) => {
                eprintln!(
                    "[FIRASim] ✗ Error enviando comando de creación de robots a {}: {}",
    ...
    pub async fn send_command(
        ...
        let buffer = serialize_to_firasim(std::slice::from_ref(cmd))?;
    ...
    pub async fn send_commands_sync(
        &self,
        commands: &[RobotCommand],
    ) -> Result<Option<SimulationSyncResponse>, Box<dyn std::error::Error + Send + Sync>> {
        ...
        if let Err(e) = self.send_commands(commands).await {
            eprintln!(
                "[FIRASim] ⚠ Error enviando comandos async (fallback): {}",
                e
            );
        }
        ...
        if !robot_control_buffer.is_empty() {
            ...
            match RobotControl::parse_from_bytes(&robot_control_buffer) {
                ...
                Err(e) => {
                    eprintln!(
                        "[FIRASim] ⚠ Error parseando RobotControl para sync request: {}",
                        e
                    );
                }
            }
        ...
        match self.socket.send_to(&request_buffer, &self.address).await {
            Ok(bytes_sent) => {
                if commands.len() <= 3 {
                    eprintln!(
                        "[FIRASim] ✓ SimulationSyncRequest enviado ({} bytes) con {} comandos",
                        bytes_sent,
                        commands.len()
                    );
                }
        ...
        .await
        {
            Ok(Ok((size, src_addr))) => {
                ...
                eprintln!(
                    "[FIRASim] ✓ Respuesta recibida: {} bytes desde {}",
                    size, src_addr
                );
                eprintln!(
                    "[FIRASim] Primeros 32 bytes (hex): {:02x?}",
                    &response_buffer[..response_buffer.len().min(32)]
                );
                ...
                match SimulationSyncResponse::parse_from_bytes(&response_buffer) {
                    Ok(response) => {
                        if !response.detection.is_empty() {
                            eprintln!(
                                "[FIRASim] ✓✓✓ SimulationSyncResponse recibido: {} frames de detección!",
                                response.detection.len()
                            );
                        } else {
                            eprintln!(
                                "[FIRASim] ⚠ SimulationSyncResponse recibido pero SIN frames de detección"
                            );
                        }
                        Ok(Some(response))
                    }
                    Err(e) => {
                        ...
                        eprintln!(
                            "[FIRASim] Buffer recibido (primeros 64 bytes): {:02x?}",
                            &response_buffer[..response_buffer.len().min(64)]
                        );
                        Ok(None)
                    }
                }
    ...
    pub async fn send_commands(
        ...
        match self.socket.send_to(&buffer, &self.address).await {
            Ok(_) => {
                ...
                if commands.len() <= 3 || n.is_multiple_of(60) {
                    eprintln!(
                        "[FIRASim] ✓ {} comandos enviados a {} (paquete #{})",
                        commands.len(),
                        self.address,
                        n
                    );
...
mod tests {
    ...
    #[test]
    fn test_firasim_client_destination_parsing() {
        let result = FIRASimClient::parse_destination("127.0.0.1", 20011);
        assert_eq!(
            result.unwrap(),
