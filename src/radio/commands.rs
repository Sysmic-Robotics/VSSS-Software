...
use crate::protos::ssl_simulation_robot_control::{
    MoveGlobalVelocity, RobotCommand as SSLRobotCommand, RobotControl, RobotMoveCommand,
};
...
pub fn serialize_commands(
    ...
    for cmd in commands {
        ...
        if cmd.team != team {
            return Err(format!(
                "Todos los comandos deben ser del mismo equipo. Encontrado equipo {} y {}",
                team, cmd.team
            )
            .into());
        }
        ...
        let max_vel = 2.0f32;
        veltangent = veltangent.clamp(-max_vel, max_vel);
        velnormal = velnormal.clamp(-max_vel, max_vel);
        ...
        let omega_limited = (cmd.motion.omega as f32).clamp(-10.0, 10.0);
    ...
    for (i, robot_cmd) in grsim_cmd.robot_commands.iter().enumerate() {
        if !robot_cmd.has_id()
            || !robot_cmd.has_veltangent()
            || !robot_cmd.has_velnormal()
            || !robot_cmd.has_velangular()
            || !robot_cmd.has_kickspeedx()
            || !robot_cmd.has_kickspeedz()
            || !robot_cmd.has_spinner()
            || !robot_cmd.has_wheelsspeed()
        {
            return Err(format!(
                "Robot command {} está incompleto: faltan campos requeridos",
                i
            )
            .into());
        }
...
pub fn serialize_to_firasim(
    ...
    for cmd in commands {
        ...
        eprintln!(
            "[serialize_to_firasim] Procesando comando: robot_id={}, team={} (nota: team no se envía en RobotCommand)",
            cmd.id, cmd.team
        );
        ...
        let omega =
            sanitize_and_clamp(cmd.motion.omega, -MOTION_OMEGA_MAX, MOTION_OMEGA_MAX) as f32; // rad/s
        ...
        if cmd.kicker.kick_x || cmd.kicker.kick_z || cmd.kicker.dribbler > 0.0 {
            ...
            let kick_angle = if cmd.kicker.kick_z { 45.0f32 } else { 0.0f32 };
    ...
    for (i, ssl_cmd) in robot_control.robot_commands.iter().enumerate() {
        ...
        if let Some(move_cmd) = ssl_cmd.move_command.as_ref()
            && !move_cmd.has_global_velocity()
        {
            return Err(
                format!("Robot command {}: falta global_velocity en move_command", i).into(),
            );
        }
    }
    ...
    if robot_control.robot_commands.len() <= 3 {
        eprintln!(
            "[serialize_to_firasim] RobotControl tiene {} comandos",
            robot_control.robot_commands.len()
        );
        for (i, cmd) in robot_control.robot_commands.iter().enumerate() {
            eprintln!(
                "[serialize_to_firasim] Comando {}: id={}, has_move_command={}, has_kick_speed={}, has_kick_angle={}, has_dribbler_speed={}",
                i,
                cmd.id(),
                cmd.move_command.as_ref().is_some(),
                cmd.has_kick_speed(),
                cmd.has_kick_angle(),
                cmd.has_dribbler_speed()
            );
            if let Some(move_cmd) = cmd.move_command.as_ref()
                && move_cmd.has_global_velocity()
            {
                let gv = move_cmd.global_velocity();
                eprintln!(
                    "[serialize_to_firasim]   global_velocity: x={:.3}, y={:.3}, angular={:.3}",
                    gv.x(),
                    gv.y(),
                    gv.angular()
                );
    ...
    match robot_control.write_to_vec(&mut buffer) {
        Ok(_) => {
            if buffer.len() <= 100 {
                eprintln!(
                    "[serialize_to_firasim] Mensaje serializado: {} bytes",
                    buffer.len()
                );
                // Mostrar primeros bytes en hex para debugging solo si el mensaje es pequeño
                eprintln!(
                    "[serialize_to_firasim] Primeros {} bytes (hex): {:02x?}",
                    buffer.len(),
                    &buffer[..]
                );
            }
...
pub fn serialize_to_fira_actuator(
    commands: &[RobotCommand],
) -> Result<Vec<u8>, Box<dyn std::error::Error + Send + Sync>> {
    if commands.is_empty() {
...
mod firasim_tests {
    ...
    fn test_serialize_to_firasim() {
        ...
        let buffer =
            serialize_to_firasim(&commands).expect("serialización FIRASim debe ser exitosa");
        assert!(!buffer.is_empty(), "Buffer serializado no debe estar vacío");
    }
