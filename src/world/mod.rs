...
impl World {
    ...
    pub fn update_robot(&mut self, id: i32, team: i32, position: Vec2, 
                       orientation: f64, velocity: Vec2, omega: f64) {
        match team {
            0 => {
                let robot = self.blue_robots.entry(id)
                    .or_insert_with(|| RobotState::new(id, team));
                robot.update(position, orientation, velocity, omega);
            }
            1 => {
                let robot = self.yellow_robots.entry(id)
                    .or_insert_with(|| RobotState::new(id, team));
                robot.update(position, orientation, velocity, omega);
            }
            _ => {}
...
mod tests {
    ...
    fn test_world_new() {
        ...
        assert_eq!(world.blue_team_size, 11);
        assert_eq!(world.yellow_team_size, 11);
    }
